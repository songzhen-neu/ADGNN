from python.adgnn.context import context
...
def breadthFirstTraversal(comp_graph, layer_count, queue,agg_emb):
    ...
    if comp_graph.operator == 'spmm':
        agg_emb['nei_embs' + str(layer_count)] = comp_graph.right.tensor.cpu().detach().numpy()
        agg_emb['agg_embs' + str(layer_count)] = comp_graph.tensor.cpu().detach().numpy()
        layer_count -= 1
    if len(queue) != 0:
