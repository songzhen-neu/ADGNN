...
def mm_pipe(o1,x1,w1):
    x2 = ecgraph.ECTensor(o1, x1, w1, 'mm', None)
    if context.glContext.is_train:
        set_requires_grad(x2, x1, w1)
        x1.root = x2
        w1.root = x2
...
def push_pipe(output,input,layer_id):
    embs = ecgraph.ECTensor(output, input, None, 'push_embs', None, requires_grad=True)
    input.root = embs
...
def spmm(adj, x1):
    ...
    x2 = ecgraph.ECTensor(x2, adj, x1, 'spmm', None)
    if context.glContext.is_train:
...
def add(x1, x2):
    ...
    x_result = ecgraph.ECTensor(x_result, x1, x2, 'add', None)
    if context.glContext.is_train:
