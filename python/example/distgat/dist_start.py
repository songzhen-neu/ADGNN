...
from example.distgat.models import GAT
...
def setGradients(model):
    with torch.no_grad():
        for id in context.glContext.parameters.keys():
            context.glContext.gradients[id]=model.parameters_collection[id].grad/context.glContext.config['worker_num']
            context.glContext.gradients[id][context.glContext.gradients[id]!=context.glContext.gradients[id]]=0
            # print(torch.max(context.glContext.gradients[id]))
            # print(torch.max(context.glContext.gradients[id]))
            context.glContext.gradients[id][torch.abs(context.glContext.gradients[id])>0.05]=0
...
class GAT_Engine(Engine):
    def run_gnn(self):
        ...
        for e in range(epoch):
            ...
            if sample_method != 'none':
                if sample_method == 'ad':
                    graph = aggDiff.sample(samp_num, e, batch_size, comm_fo=comm_fo, enable_pc=enable_pc, m=m_ad,
                                           ...
                                           model=self.model,model_type='GAT')
            ...
            time_counter.end('fp')
            loss_train = F.nll_loss(output, graph.subgraphs['train'].label.tensor.to(context.glContext.config['device']))
            acc_train = accuracy_score(graph.subgraphs['train'].label.tensor,
            ...
            if e % context.glContext.config['print_result_interval'] == 0:
                ...
                acc_val = accuracy_score(context.glContext.graph_full.subgraphs['val'].label.tensor,
                                         output.cpu().detach().numpy().argmax(axis=1))
                ...
                context.glContext.graph_full.subgraphs['test'].label.tensor,
                output.cpu().detach().numpy().argmax(axis=1))
...
if __name__ == "__main__":
    ...
    model = GAT(nfeat=context.glContext.config['feature_dim'],
                ...
                dropout=0.6,
                alpha=0.005,
                nheads=context.glContext.config['head'])
    gat_engine = GAT_Engine(model)
    gat_engine()
