...
class GraphConvolution(nn.Module):
    def __init__(self, in_features, out_features, layer_id, bias=True):
        ...
        glContext.parameters['w' + str(layer_id)] = self.weight.data.flatten().detach().tolist()

        if bias:
            self.bias = nn.Parameter(torch.empty(size=(1, out_features)))
            nn.init.zeros_(self.bias)
            glContext.parameters['b' + str(layer_id)] = self.bias.data.flatten().detach().tolist()
    ...
    def pushEmbs(self,layer_id, graph, input):
        embs=glContext.dgnnClientRouterForCpp.pushEmbs(layer_id,graph.status,graph.graph_mode,input.detach().numpy())
        embs = torch.FloatTensor(embs)
        return embs

    def inference_layer(self,input,graph):
        time_counter.start("fp-getadj")
        adj = graph.getAdj(self.layer_id).tensor
        time_counter.end("fp-getadj")
        # embs_trans = F.mm(input, self.weight)
        # embs = rmt.pushEmbs(self.layer_id, graph, embs_trans)
        # output = F.spmm(adj, embs)
        if self.in_features>self.out_features:
            ...
            time_counter.end("fp-mm")
            time_counter.start("fp-push")
            embs = self.pushEmbs(self.layer_id, graph, embs_trans)
            time_counter.end("fp-push")
            time_counter.start("fp-spmm")
            output = torch.spmm(adj, embs)
            time_counter.end("fp-spmm")
        else:
            time_counter.start("fp-push")
            embs = self.pushEmbs(self.layer_id, graph, input)
            time_counter.end("fp-push")
            time_counter.start("fp-spmm")
            output = torch.spmm(adj, embs)
            time_counter.end("fp-spmm")
