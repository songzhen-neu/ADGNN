...
vector<map<int, unordered_set<int>>> GeneralPartition::adjs;
unordered_map<int,int> GeneralPartition::degree_map;
...
//    }
    ...
    for(int i=0;i<adj_map.size();i++){
        GeneralPartition::degree_map.insert(make_pair(i,adj_map[i].size()));
    }
    cout<<"GeneralPartition::degree_map size:"<<GeneralPartition::degree_map.size()<<endl;
