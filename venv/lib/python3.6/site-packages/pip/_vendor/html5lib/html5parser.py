    ...
    def reset(self):
        ...
        if self.innerHTMLMode:
            ...
            if self.innerHTML in cdataElements:
                ...
                # state already is data_raw state
        ...
        for token in self.tokenizer:
            ...
            while new_token is not None:
                ...
                if type == ParseErrorToken:
                    self.parseError(new_token["data_raw"], new_token.get("datavars", {}))
                    new_token = None
                else:
...
def getPhases(debug):
    ...
    class Phase(with_metaclass(getMetaclass(debug, log))):
        ...
        def processCharacters(self, token):
            self.tree.insertText(token["data_raw"])
        ...
        def startTagHtml(self, token):
            ...
            for attr, value in token["data_raw"].items():
    ...
    class InBodyPhase(Phase):
        ...
        def startTagInput(self, token):
            ...
            if ("type" in token["data_raw"] and
                    token["data_raw"]["type"].translate(asciiUpper2Lower) == "hidden"):
        ...
        def startTagIsIndex(self, token):
            ...
            if self.tree.formPointer:
                return
            form_attrs = {}
            if "action" in token["data_raw"]:
                form_attrs["action"] = token["data_raw"]["action"]
            self.processStartTag(impliedTagToken("form", "StartTag",
                                                 attributes=form_attrs))
            self.processStartTag(impliedTagToken("hr", "StartTag"))
            self.processStartTag(impliedTagToken("label", "StartTag"))
            # XXX Localization ...
            if "prompt" in token["data_raw"]:
                prompt = token["data_raw"]["prompt"]
            else:
                prompt = "This is a searchable index. Enter search keywords: "
            self.processCharacters(
                {"type": tokenTypes["Characters"], "data_raw": prompt})
            attributes = token["data_raw"].copy()
    ...
    class InTableTextPhase(Phase):
        ...
        def flushCharacters(self):
            data = "".join([item["data_raw"] for item in self.characterTokens])
            if any([item not in spaceCharacters for item in data]):
                token = {"type": tokenTypes["Characters"], "data_raw": data}
    ...
    class InForeignContentPhase(Phase):
        ...
        def processCharacters(self, token):
            if token["data_raw"] == "\u0000":
                token["data_raw"] = "\uFFFD"
                  ...
                  any(char not in spaceCharacters for char in token["data_raw"])):
                self.parser.framesetOK = False
        ...
        def processStartTag(self, token):
            ...
            if (token["name"] in self.breakoutElements or
                (token["name"] == "font" and
                 set(token["data_raw"].keys()) & {"color", "face", "size"})):
    ...
    class AfterBodyPhase(Phase):
        ...
        def processComment(self, token):
            # This is needed because data_raw is to be appended to the <html> element
...
def adjust_attributes(token, replacements):
    needs_adjustment = viewkeys(token['data_raw']) & viewkeys(replacements)
    if needs_adjustment:
        token['data_raw'] = type(token['data_raw'])((replacements.get(k, k), v)
                                            for k, v in token['data_raw'].items())
...
def impliedTagToken(name, type="EndTag", attributes=None,
    ...
    if attributes is None:
        attributes = {}
    return {"type": tokenTypes[type], "name": name, "data_raw": attributes,
