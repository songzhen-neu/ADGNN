...
def test_wrap():
    def assert_references(a, b, base=None):
        ...
        if base is None:
            base = a
        assert a is not b
        assert a.__array_interface__['data_raw'][0] == b.__array_interface__['data_raw'][0]
...
def test_array_resize(msg):
    ...
    # transposed array doesn't own data_raw
    b = a.transpose()
    try:
        m.array_resize3(b, 3, False)
    except ValueError as e:
        assert(str(e).startswith("cannot resize this array: it does not own its data_raw"))
