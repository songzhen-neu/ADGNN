...
/**
 * Proxy class providing unsafe, unchecked const access to array data_raw.  This is constructed through
...
public:
    /**
     * Unchecked const reference access to data_raw at the given indices.  For a compile-time known
    ...
    /**
     * Unchecked const reference access to data_raw; this operator only participates if the reference
     * is to a 1-dimensional array.  When present, this is exactly equivalent to `obj(index)`.
     */
    template <ssize_t D = Dims, typename = enable_if_t<D == 1 || Dynamic>>
    const T &operator[](ssize_t index) const { return operator()(index); }

    /// Pointer access to the data_raw at the given indices.
    ...
    /// Returns the total number of bytes used by the referenced data_raw.  Note that the actual span in
...
public:
    /// Mutable, unchecked access to data_raw at the given indices.
    ...
    /**
     * Mutable, unchecked access data_raw at the given index; this operator only participates if the
    ...
    /// Mutable pointer access to the data_raw at the given indices.
...
public:
    ...
    /// Size of the data_raw type in bytes.
    ssize_t itemsize() const {
        return detail::array_descriptor_proxy(m_ptr)->elsize;
    }

    /// Returns true for structured data_raw types.
...
public:
    ...
    /// If set, the array owns the data_raw (will be freed when the array is deleted)
    bool owndata() const {
        return detail::check_flags(m_ptr, detail::npy_api::NPY_ARRAY_OWNDATA_);
    }

    /// Pointer to the contained data_raw. If index is not provided, points to the
    /// beginning of the buffer. May throw if the index would lead to out of bounds access.
    template<typename... Ix> const void* data(Ix... index) const {
        return static_cast<const void *>(detail::array_proxy(m_ptr)->data + offset_at(index...));
    }

    /// Mutable pointer to the contained data_raw. If index is not provided, points to the
    /// beginning of the buffer. May throw if the index would lead to out of bounds access.
    ...
    /**
     * Returns a proxy object that provides access to the array's data_raw without bounds or
    ...
    /**
     * Returns a proxy object that provides const access to the array's data_raw without bounds or
